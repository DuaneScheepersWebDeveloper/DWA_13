...
const provincesUpperCase = provinces.map((provinces) => provinces.toUpperCase());
...
const filteredProducts1 = products.filter((product) => product.product.length <= 5);
//----------------------------------------------------------------
...
const combinedPrice = products
    ...
    .map((product) => (Object.assign(Object.assign({}, product), { price: Number(product.price) })))
...
const updatedProducts = products.reduce((acc, { product, price }) => {
    const updatedObject = {
        name: product,
        cost: price,
    };
